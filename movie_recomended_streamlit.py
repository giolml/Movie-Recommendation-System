...
def preprocesamiento():
    ...
    df_movies = df_movies.drop_duplicates(subset=['movieId', 'title'])
...
# Eliminamos los duplicados de 'df_final'
df_final_sin_duplicados = df_final.drop_duplicates(subset='title', keep='first')
...
# Función de recomendación ponderada o promedio bayesiano
...
# Función para buscar coincidencias más cercanas
def buscar_pelicula_aproximada(titulo_input, titulos_peliculas, n=5):
    coincidencias = difflib.get_close_matches(titulo_input, titulos_peliculas, n=n)
    if coincidencias:
...
def main():
    ...
    # Agregar una imagen en la barra lateral antes del menú de navegación
    st.sidebar.image("BX.png", use_column_width=True)
    ...
    if choice == "Inicio":
        ...
        st.write("- **Giovanni Roncancio**: Data Scientist")
        st.write("- **Mirna Prieto**: Data Scientist")
        st.write("- **Lucel Da Silva**: Data Scientist")
        st.write("- **Juan Campos**: Data Scientist")
        st.write("- **Wilfer Alexander**: Data Scientist")
        st.image("G3.png", caption="Grupo 3 - Recomendador de Películas", use_column_width=True)
        st.write("Bienvenido al Sistema de Recomendación de Películas Moviesuggest.")
        ...
        st.title("Qué hace el proyecto.")
        st.write("Este proyecto toma los datos de películas y sus raitings de una plataforma publicados en la web. Los analiza y aplica filtros para determinar los títulos mas votados. Buscando similutudes en los comportamientos de los usuarios que hayan expresados sus votos, para recomendarle películas de estilo similar aproximado a sus preferencias.")
        st.title("Desafío del proyecto")
        st.write("¡Imagina tener acceso a los datos de una plataforma de streaming y usar esos datos para mejorar la experiencia del usuario! Como científicos de datos, analizamos estos datos con Python y construimos un recomendador de películas personalizado que ofrecerá sugerencias precisas y relevantes, revolucionando la forma en que los usuarios descubren nuevos contenidos.")
